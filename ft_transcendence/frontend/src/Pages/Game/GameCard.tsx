...
interface NewGameCardProps {
    status?: "default" | "waiting" | "disabled";
}

const NewGameCard: React.FC<NewGameCardProps> = ({ status = "default" }) => {
    const { user } = useAuth();
    ...
    return (
        <Col xl={6} className="margin-card position-card-game" style={{ marginBottom: "30px" }}>
            {status === "waiting" && <img className="close-icon" src={CloseIcon} alt="Close" />}
            <div className='card-settings game-inline-card inline-game-card auth-shadow'>
                ...
                <div className='left full-width game-vertical-align mobile-game-card'>
                    <Col xl={6}>
                        <GameTypeBox icon={QuanityIcon} text="Up to 3 wins" />
                        <GameTypeBox icon={UpIcon} text="Power-up Mode" />
